...
public class AccountController {
    ...
    String create(@Validated AccountForm form, BindingResult bindingResult) {
        ...
        // 権限を設定
        accountEntity.setRoleId(Integer.parseInt(Contains.RoleId.ADMIN.getValue()));
        // 更新者を設定
        accountEntity.setUpdaterId(1);
        // 作成日時を設定
        accountEntity.setCreated(TimestampUtil.currentTime());
        // 更新日時を設定
        accountEntity.setUpdated(TimestampUtil.currentTime());
        // 削除フラグを設定
        accountEntity.setDelflg(Contains.DelFlg.NOT_DEL.getValue());
